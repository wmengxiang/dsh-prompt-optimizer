...
window.__ModuleLoader__.load({
  ...
  factory(require) {
    ...
    const CSS = [
      ...
      ".dsh-po-btn:disabled{opacity:.95;cursor:not-allowed;filter:saturate(.45)}",
      ".dsh-po-btn.dsh-po-busy:disabled{opacity:1;cursor:progress;filter:none}",
    ...
    function apply(ctx) {
      // 幂等样式注入：存在则刷新内容（HMR 只重放模块不重载页面时，旧写法会让新 CSS 失效）。
